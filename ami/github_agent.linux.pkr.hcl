...
build {
  ...
  provisioner "shell" {
    ...
    inline = concat([
      ...
      "sudo dnf install -y amazon-cloudwatch-agent jq git docker nodejs unzip",
  ...
  provisioner "shell" {
    environment_vars = [
      "RUNNER_TARBALL_URL=https://github.com/actions/runner/releases/download/v2.327.1/actions-runner-linux-x64-2.327.1.tar.gz"
    ]
    inline = [
  ...
  provisioner "file" {
    content = templatefile("./start-runner.sh", {
      start_runner = templatefile("./templates/start-runner.sh", { metadata_tags = "enabled" })
    })
